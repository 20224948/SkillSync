...
export default function DashboardPage() {
  ...
  const studentName =
    reportData?.student?.display_name || report?.student?.name || 'Student'
  ...
  const strengths = [...silos]
    ...
    .sort((a: any, b: any) => Number(b.mastery_score) - Number(a.mastery_score))
    .slice(0, 3)
...
>
                ...
                <div className="dashboard-list-row" key={index}>
                  ...
                  <div className="dashboard-progress">
                    <div
                      ...
                      style={{ width: `${Number(item.mastery_score)}%` }}
                    />
                  </div>
                </div>
              ))
      ...
      <div className="soft-card learning-overview-card">
        <h3 className="metric-title">Current Learning Overview</h3>
        ...
        <div className="learning-overview-list">
          {silos.length === 0 ? (
            ...
            silos.map((silo: any, index: number) => (
              <div className="learning-overview-item" key={index}>
                <span className="learning-overview-title">
                  {silo.competency_id} - {cleanTitle(silo.title)}
                </span>

                <div className="dashboard-progress">
                  <div
                    className="dashboard-progress-fill"
                    style={{ width: `${Number(silo.mastery_score)}%` }}
                  />
                </div>
                ...
                <span className="learning-overview-score">
                  {silo.previous_mastery_score !== null &&
                    ...
                    >
                      {Number(silo.previous_mastery_score).toFixed(0)}% →{' '}
                      {Number(silo.mastery_score).toFixed(0)}% (
                      {Number(silo.mastery_change) >= 0 ? '+' : ''}
                      {Number(silo.mastery_change).toFixed(0)}%)
                    </span>
                  ) : (
                    <span>{Number(silo.mastery_score).toFixed(0)}%</span>
                  )}
                </span>
              </div>
            ))
